...
class TableModel {
    constructor(tableName, identifierColumnName, identifierColumnValue) {
        this.tableName = connection.escapeId(tableName);
        this.identifierColumnName = connection.escapeId(identifierColumnName);
    ...
    async getRowsPerColumn(column, value) {
        return await connection.sqlSelect(`
            SELECT * 
            FROM ${this.tableName}
            WHERE ${connection.escapeId(column)} = :value
        `, { value: value })
    }
